...
class Step9_CustomSpec
  extends TestKit(ActorSystem("Step9CustomSpec"))
  with FunSuite
  ...
  test("case2: Random ops with 3 secondaries") {
    ...
    for (i <- 0 until 1000) {
      randomQuery(client)
      if      (i == 100) arbiter.send(primary, Replicas(Set(primary, secondaries(0))))
      else if (i == 200) arbiter.send(primary, Replicas(Set(primary, secondaries(0), secondaries(1))))
      else if (i == 300) arbiter.send(primary, Replicas(Set(primary, secondaries(0), secondaries(1), secondaries(2))))
      else if (i == 400) arbiter.send(primary, Replicas(Set(primary, secondaries(0), secondaries(1))))
      else if (i == 500) arbiter.send(primary, Replicas(Set(primary, secondaries(0))))
      else if (i == 600) arbiter.send(primary, Replicas(Set(primary)))
    }
  }
  ...
  test("case3: Random ops with multiple clusters") {
    ...
    for (i <- 0 until 1000) {
      randomQuery(client)
      if (randomInt(10) < 3)
        arbiter.send(primary, Replicas(Set(primary) ++ cluster(randomInt(10))))
    }
