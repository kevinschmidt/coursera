...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.toStream.map { case (b, m) =>
    (b, m :: history)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, history) #:: rest => (block, history) #:: from(
      rest ++ newNeighborsOnly(neighborsWithHistory(block, history), explored),
      explored + block
    )
  }
