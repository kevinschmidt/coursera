...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(highest: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(highest: Tweet): Tweet = highest
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val result = if (p(elem)) acc.incl(elem) else acc
    left.filterAcc(p, right.filterAcc(p, result))
  }
  ...
  def mostRetweetedAcc(highest: Tweet): Tweet = {
    val result = if (elem.retweets > highest.retweets) elem else highest
    left.mostRetweetedAcc(right.mostRetweetedAcc(result))
  }
